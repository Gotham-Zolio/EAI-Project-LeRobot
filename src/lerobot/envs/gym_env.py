...
class LeRobotGymEnv(gym.Env):
    def __init__(self, task="lift", headless=True, max_steps=500, base_seed=None):
        super().__init__()
        ...
        # 记录 base_seed（用于扰动每个 episode 的 seed）
        if base_seed is None:
            import os
            self.base_seed = int.from_bytes(os.urandom(8), 'big') % 2**32
        else:
        ...
        self.task_actors = setup_scene(self.scene, self.task, rng=None)
    ...
    def reset(self, seed=None, options=None):
        ...
        # 使用 base_seed + seed 创建独立的 numpy 随机生成器，避免全局 seed 导致重复
        if seed is not None:
            episode_seed = self.base_seed + seed
        else:
        ...
        # Re-populate scene (randomized)，将 rng 传递给 setup_scene
        self.task_actors = setup_scene(self.scene, self.task, rng=rng)
